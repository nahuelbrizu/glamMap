...
import { FiCamera, FiUser, FiPhone, FiMail, FiLock, FiSave } from 'react-icons/fi';
        ...
        <div className="flex flex-col items-center pt-8 pb-6">
          <div className="relative">
            <div className="w-32 h-32 rounded-full overflow-hidden border-4 border-white dark:border-slate-800 shadow-xl">
              <img
                src={user?.avatar_url || `https://ui-avatars.com/api/?name=${encodeURIComponent(formData.name)}&background=13c8ec&color=fff`}
                alt="Profile"
            ...
            <button
              type="button"
              className="absolute bottom-1 right-1 bg-primary text-white p-2 rounded-full shadow-lg border-2 border-background-light dark:border-background-dark hover:scale-110 transition-transform"
              aria-label="Cambiar foto"
            >
              <FiCamera size={18} />
            </button>
          </div>
          <button type="button" className="mt-4 text-primary font-bold text-sm">Cambiar foto</button>
        </div>
